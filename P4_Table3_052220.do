...
gl exceloutput "${boxfolder}\Analysis\Tables\Table shells 052220.xlsx"
putexcel set "$exceloutput", modify sheet("Table 3")
...
global varlist anyed morethantwoed anyop morethantwoop anyip morethantwoip anyvisit twoormoreanyvisit threeormoreanyvisit ///
			   usetob usealc usedrugs advqttob advcutalc doctxalc ///
			   txyrrecvd2 txyrill txyralc notxalcdrg noneednotx effnotx noeffnotx txoveryr ///
...
foreach var in $varlist {
		...
		*Only skip a row after certain variables
		if inlist("`var'", "threeormoreanyvisit", "usedrugs", "doctxalc", "txoveryr") { 
			local ++row
			sleep 500
		}
